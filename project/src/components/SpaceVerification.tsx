...
const SpaceVerification = () => {
  ...
  // ================= VALIDATION =================
  const allConfirmed =
    Object.values(confirmations).every(Boolean) && photoTaken;

  const submitVerification = () => {
    if (!allConfirmed) {
      setError('You must complete ALL requirements to proceed.');
      return;
    }
     ...
     * - Send photo + confirmations to backend
  ...
  return (
    <div className="min-h-screen bg-blue-50 flex items-center justify-center p-4">
      <div className="bg-white max-w-4xl w-full p-6 rounded-xl shadow-xl">

        <div className="text-center mb-4">
          <Camera className="w-10 h-10 mx-auto text-blue-600" />
          <h1 className="text-2xl font-bold">Exam Space Verification</h1>
          <p className="text-gray-600 text-sm">
            Manual verification required before accessing the exam
          </p>
        </div>

        {error && (
          <div className="bg-red-100 border border-red-300 p-3 rounded mb-4 text-red-700 flex items-center">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            {!photoTaken && cameraReady && (
              <button
                ...
                className="mt-3 w-full bg-blue-600 hover:bg-blue-700 text-white py-2 rounded font-semibold"
              >
            ...
            {photoTaken && (
              <div className="mt-3 text-green-700 flex items-center">
          ...
          {/* Confirmations */}
          <div>
            <h2 className="font-semibold mb-2">Confirm ALL of the following:</h2>

            {[
              { key: 'alone', text: 'I am alone in the room' },
              { key: 'noPapers', text: 'I have no books or papers' },
              { key: 'noDevices', text: 'I have no phone, laptop, or second device' },
              { key: 'quiet', text: 'I am in a quiet environment' },
              { key: 'honest', text: 'I understand violations lead to disqualification' },
            ].map(item => (
              <label key={item.key} className="flex items-start mb-2">
                <input
                  type="checkbox"
                  className="mt-1 mr-2"
                  checked={(confirmations as any)[item.key]}
                  onChange={e =>
            ...
            >
              {submitting ? 'Submitting...' : 'Proceed to Instructions'}
            </button>
          </div>
        </div>
      </div>
