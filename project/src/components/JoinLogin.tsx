...
export default function Login(): JSX.Element {
  ...
  const [loginType, setLoginType] = useState<"admin" | "lecturer" | null>(null);
  ...
  const handleAdminLogin = async () => {
    ...
    try {
      ...
      setTokenMessage(adminResponse.data.message);
      setLoginType("admin");
      ...
      try {
        ...
        setTokenMessage(lecturerResponse.data.message);
    ...
    <div className="font-['Roboto'] flex justify-center items-center min-h-screen bg-[#ebebeb] py-5">
      <div className="relative w-[420px] bg-white text-[#106053]">
        ...
        <div className="w-full p-10">
            ...
            <form onSubmit={handleStudentLogin}>
              ...
              <div className="relative w-full h-[50px] bg-[#d6d6d6] mb-8">
                <select
                  value={semester}
                  onChange={(e) => setSemester(e.target.value)}
                  className="w-full h-full bg-transparent border-none outline-none border-[2px] border-white/10 rounded-[40px] text-[16px] text-black px-[20px] pr-[45px]"
                >
                  <option value="" disabled>Select Semester</option>
                  <option value="1">Semester 1</option>
                  <option value="2">Semester 2</option>
          ...
          {errorMessage && <p className="text-red-600 font-bold mt-2 text-center text-sm">{errorMessage}</p>}
          {successMessage && <p className="text-green-600 font-bold mt-2 text-center text-sm">{successMessage}</p>}
        </div>
      </div>
      ...
      <Dialog
        ...
        onClose={() => {
          ...
          if (loginType === "admin") navigate("/cleartoken");
          if (loginType === "lecturer") navigate("/verify-token");
        }}
      >
        <DialogTitle className="text-green-700 font-bold">Token Sent</DialogTitle>
        <DialogContent className="text-gray-700">{tokenMessage}</DialogContent>
        <DialogActions>
          {/* <Button
            onClick={() => {
              setTokenDialogOpen(false);
  ...
  variant="contained"
  className="!bg-[#106053] hover:!bg-[#0b3f37] !text-white font-bold"
>
  Continue
</Button>
